...
func TestRemoteDequeue(t *testing.T) {
	// ---------------------------------------------------------------------------
	// Setup the nodes
	// ---------------------------------------------------------------------------
	...
	// Dequeue from other node
	bodyStruct := HTTPDequeueRequest{
		Topic:              "topic1",
	...
	// Dequeue from node 2
	req, err = http.NewRequest("POST", fmt.Sprintf("http://localhost:%s/dequeue", httpPort2), bytes.NewReader(b))
	if err != nil {
		t.Fatal(err)
	}
